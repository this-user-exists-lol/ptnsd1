...
if (instance_exists(baddieID) && place_meeting(x, y, obj_player1) && obj_player1.cutscene == 0 && obj_player1.state != states.firemouth)
{
    if (baddieID.state != states.grabbed)
    {
        with (obj_player1)
        {
            ...
            if (instance_exists(other.baddieID) && other.baddieID.state != states.throw2 && other.baddieID.vsp > 0 && state != states.tackle && state != states.superslam && state != states.machslide && state != states.freefall && state != states.mach2 && state != states.handstandjump)
            {
