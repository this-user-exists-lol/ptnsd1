...
if (floor(image_index) == 10)
    shoving = 0
if (((room == rank_room) || ((room == timesuproom) || ((room == Realtitlescreen) || (room == characterselect)))))
    visible = false
else
    visible = true
if (global.combo >= 10)
...
if (instance_exists(player) && ((player.state != 36) && (player.state != 55)))
{
	...
	if (player.vsp > 15) && (vspcamera < 100)
		vspcamera += 1
	else if (player.vsp < -15) && (vspcamera > -100)
		vspcamera -= 1
	else
	{
        if (vspcamera > 0)
	...
	__view_set(1, 0, (((target.y - (__view_get(3, 0) / 2)) + vspcamera) + p2pdistancex))
